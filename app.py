...
<style>
    ...
    .streamlit-expanderContent p, .stMarkdown p { font-size: 1.2rem !important; }
...
# เราต้องเก็บข้อมูลใน Session State เพื่อให้เพิ่มหุ้นแล้วค่าไม่หาย
...
# --- 3. Sidebar Settings & Add Stock Function ---
with st.sidebar:
    ...
    with st.form("add_stock_form"):
        new_ticker = st.text_input("Ticker Symbol (e.g. MSFT)").upper()
        new_qty = st.number_input("Quantity", min_value=0.0001, format="%.4f")
        new_cost = st.number_input("Avg Cost ($)", min_value=0.01, format="%.2f")
        new_type = st.selectbox("Category", ["Growth", "Defensive"])
        
        submitted = st.form_submit_button("Submit Order")
        
        if submitted and new_ticker:
            # 1. Add to Portfolio
            new_entry = {"Ticker": new_ticker, "Category": new_type, "Avg Cost": new_cost, "Qty": new_qty}
            ...
            if new_ticker in st.session_state.watchlist:
                ...
                st.success(f"Added {new_ticker} to {new_type} & Removed from Watchlist!")
            else:
                st.success(f"Added {new_ticker} to {new_type}!")
            
            # 3. Clear Cache to fetch new data
            st.cache_data.clear()
...
try:
    ...
    with col_bot_left:
        ...
        df_growth = df[df['Category'] == 'Growth'].copy() # Dynamic Filtering
        ...
        df_defensive = df[df['Category'] == 'Defensive'].copy() # Dynamic Filtering
    ...
    with col_bot_right:
        ...
        watchlist_data = []
        # Use Dynamic Watchlist from Session State
    ...
    st.error(f"System Error: {e}")import streamlit as st
import pandas as pd
import yfinance as yf
from datetime import datetime, timedelta
import plotly.graph_objects as go
...
try:
    ...
    with col_bot_left:
        ...
        st.dataframe(
            ...
            .map(color_text, subset=['% P/L', 'Total Gain USD'])
            .map(color_diff_s1_logic, subset=['Diff S1']),
            # [NEW COLUMN HEADERS]
            column_order=["Ticker", "Company", "Qty", "Avg Cost", "Total Cost", "% P/L", "Current Price", "Value USD", "Total Gain USD", "Diff S1", "Buy Lv.1", "Buy Lv.2", "Sell Lv.1", "Sell Lv.2"],
            column_config={
                "Current Price": "Price", 
                "% P/L": "% Total", 
                "Value USD": "Value ($)", 
